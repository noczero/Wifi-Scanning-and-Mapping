...
class AccessPoint(dict):
    ...
    def __init__(self, ssid, bssid, quality, security, rssi):
        dict.__init__(self, ssid=ssid, bssid=bssid, quality=quality, security=security, rssi=rssi)
        ...
        for line in output.split("\n"):
            if line.strip().startswith("SSID BSSID"):
                ...
                try:
                    ...
                    ap = AccessPoint(ssid, bssid, rssi_to_quality(int(rssi)), security, rssi)
                    results.append(ap)
    ...
    def parse_output(self, output):
        ...
        for num, line in enumerate(output.split("\n")):
            ...
            if line.startswith("SSID"):
                ...
                rssi = quality_to_rssi(quality)
                if bssid is not None:
                    ap = AccessPoint(ssid, bssid, quality, security, rssi)
                    results.append(ap)
        return results
    ...
    def parse_output(self, output):
        ...
        for line in output.strip().split('\n'):
            try:
                ...
                continue
            access_point = AccessPoint(ssid, bssid, int(quality), security, quality_to_rssi(quality))
    ...
    def parse_output(self, output):
        ...
        for num, line in enumerate(output.split("\n")):
            ...
            if line.startswith("Cell"):
                if bssid is not None:
                    ap = AccessPoint(ssid, bssid, quality, security, quality_to_rssi(quality))
                    results.append(ap)
                    security = []
                bssid = ":".join(line.split(":")[1:]).strip()
                bssid_line = num
